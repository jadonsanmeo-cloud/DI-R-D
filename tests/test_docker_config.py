...
class DockerConfigTests(unittest.TestCase):
    ...
    def test_dockerfile_installs_locked_dependencies_with_uv(self) -> None:
        ...
        self.assertIn("COPY pyproject.toml uv.lock README.md ./", content)
        self.assertIn("uv sync --frozen --no-dev --no-editable", content)
    ...
    def test_compose_contains_backend_api_and_private_postgres(self) -> None:
        ...
        self.assertIn("DATA_CORPUS_ROOT: /app", content)
        ...
        self.assertIn("MODEL_CONFIG_PATH: /app/configs/proxy-openrouter.toml", content)
