...
// Lazy-loaded: RecallTestModal pulls in the markdown/chart/vis stack
// (MarkdownContext -> chat-content/config -> @antv/g2, @ant-design/plots,
// @antv/gpt-vis, katex, react-syntax-highlighter, ...). It is a controlled
// modal that only renders on demand, so we defer loading it off the route's
// initial compile graph.
const RecallTestModal = dynamic(() => import('./RecallTestModal'), {
  ssr: false,
  loading: () => null,
});
                  ...
                  {fields.map(({ key, name }) => (
                    <div key={key} className={cls('flex flex-1 items-center gap-8 mb-6')}>
                      <Form.Item label='' name={[name, 'question']} className='grow'>
                        <Input placeholder={t('kn.pleaseInput')} />
                      </Form.Item>
                      <Form.Item>
                        <MinusCircleOutlined
                          onClick={() => {
