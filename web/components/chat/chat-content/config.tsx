import type { BackEndChartType } from '@/components/chart';
...
import type VisResponseType from './VisResponse';
import type AgentMessagesType from './agent-messages';
import type AgentPlansType from './agent-plans';
import { CodePreview } from './code-preview';
import type VisChartType from './vis-chart';
import type VisCodeType from './vis-code';
import type VisConvertErrorType from './vis-convert-error';
import type VisDashboardType from './vis-dashboard';
import type VisPluginType from './vis-plugin';
...
const HtmlPreview = dynamic(() => import('./html-preview'), { ssr: false });
const SvgPreview = dynamic(() => import('./svg-preview'), { ssr: false });
const VisChart = dynamic(() => import('./vis-chart'), { ssr: false });
const VisCode = dynamic(() => import('./vis-code'), { ssr: false });
const VisConvertError = dynamic(() => import('./vis-convert-error'), { ssr: false });
const VisDashboard = dynamic(() => import('./vis-dashboard'), { ssr: false });
const VisPlugin = dynamic(() => import('./vis-plugin'), { ssr: false });
  ...
  code: withDefaultChartCode({
    languageRenderers: {
      'agent-plans': ({ className, children }) => {
        ...
        try {
          const data = JSON.parse(content) as Parameters<typeof AgentPlansType>[0]['data'];
      ...
      'agent-messages': ({ className, children }) => {
        ...
        const lang = className?.replace('language-', '') || 'javascript';
        try {
          const data = JSON.parse(content) as Parameters<typeof AgentMessagesType>[0]['data'];
      ...
      'vis-convert-error': ({ className, children }) => {
        ...
        const lang = className?.replace('language-', '') || 'javascript';
        try {
          const data = JSON.parse(content) as Parameters<typeof VisConvertErrorType>[0]['data'];
      ...
      'vis-dashboard': ({ className, children }) => {
        ...
        const lang = className?.replace('language-', '') || 'javascript';
        try {
          const data = JSON.parse(content) as Parameters<typeof VisDashboardType>[0]['data'];
      ...
      'vis-db-chart': ({ className, children }) => {
        ...
        const lang = className?.replace('language-', '') || 'javascript';
        try {
          const data = JSON.parse(content) as Parameters<typeof VisChartType>[0]['data'];
      ...
      'vis-plugin': ({ className, children }) => {
        ...
        const lang = className?.replace('language-', '') || 'javascript';
        try {
          const data = JSON.parse(content) as Parameters<typeof VisPluginType>[0]['data'];
      ...
      'vis-code': ({ className, children }) => {
        ...
        try {
          const data = JSON.parse(content) as Parameters<typeof VisCodeType>[0]['data'];
      ...
      'vis-api-response': ({ className, children }) => {
        ...
        const lang = className?.replace('language-', '') || 'javascript';
        try {
          const data = JSON.parse(content) as Parameters<typeof VisResponseType>[0]['data'];
...
const extraComponents: MarkdownComponent = {
  'chart-view': function ({ content, children }) {
    ...
    return (
      <div>
        <ChartView data={data?.data} type={data?.type} sql={returnSqlVal(data?.sql ?? '')} />
        {children}
      </div>
    );
  },
