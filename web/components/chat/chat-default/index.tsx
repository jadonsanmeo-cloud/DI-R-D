            ...
            <span className='flex items-center text-gray-500 gap-1 dark:text-slate-300'>
              ...
              >
                <Image
                  ...
                  height={24}
                />
                <span className='text-default'>{t('cmp.exploreSquare')}</span>
              </span>
              <span>{t('cmp.discoverMore')}</span>
            </span>
          </div>
          <TabContent apps={apps?.app_list || []} loading={loading} refresh={refresh} />
          {helps && helps.length > 0 && (
            <div>
              <h2 className='font-medium text-xl my-4'>{t('cmp.iCanHelpYou')}</h2>
