...
export function useConnectors(options?: { manual?: boolean }) {
  ...
  const fetchConnectors = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(API_BASE_LIST);
  ...
  }, [fetchConnectors, options?.manual, version]);
  ...
  const refresh = useCallback(() => {
    ...
    setVersion(v => v + 1);
  }, [fetchConnectors, options?.manual]);

  return { connectors, loading, refresh, fetch: fetchConnectors };
}
