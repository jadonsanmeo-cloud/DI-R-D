...
const Feedback: React.FC<{
  ...
  const onCopyContext = async (context: any) => {
    ...
    if (result) {
      if (pureStr) {
        message.success(t('mob.copySuccess'));
      } else {
        message.warning(t('mob.copyEmpty'));
      }
    } else {
      message.error(t('mob.copyFailed'));
    }
  };
  ...
  const { run: feedback, loading } = useRequest(
      ...
      ),
    {
      ...
      onSuccess: data => {
        ...
        message.success(t('mob.feedbackSuccess'));
  ...
  const { run: cancel } = useRequest(
    async () => await apiInterceptors(cancelFeedback({ conv_uid: conv_uid, message_id: content?.order + '' })),
    {
      ...
      onSuccess: data => {
        ...
        if (res) {
          setStatus('none');
          message.success(t('mob.operationSuccess'));
        }
      },
    },
  );
