...
function buildSections(steps: ManusExecutionStep[]): ThinkingSection[] {
  ...
  if (thinkSteps.length > 0)
    sections.push({ id: 'section-think', title: i18n.t('pg.sectionPlanning'), isCompleted: true, steps: thinkSteps });
  if (skillSteps.length > 0)
    sections.push({ id: 'section-skill', title: i18n.t('pg.sectionSkillLoading'), isCompleted: true, steps: skillSteps });
  if (otherSteps.length > 0)
    sections.push({ id: 'section-execution', title: i18n.t('pg.sectionExecution'), isCompleted: true, steps: otherSteps });
  if (sections.length === 0) sections.push({ id: 'section-main', title: i18n.t('pg.sectionProcess'), isCompleted: true, steps });
  return sections;
}
...
const SharePage: NextPage = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/api/v1/chat/share/${token}`)
      .then(res => {
        if (!res.ok) {
          setFetchError(t('pg.shareLinkInvalidStatus', { status: res.status }));
          setLoading(false);
          return null;
        }
        return res.json();
      ...
      .then(json => {
        ...
        if (!Array.isArray(rawMessages)) {
          setFetchError(t('pg.dataFormatError'));
          setLoading(false);
          return;
        }
      ...
      .catch((err: any) => {
        setFetchError(err?.message || t('pg.loadFailed'));
        setLoading(false);
      });
  }, [token]);
  ...
  const handleCopyLink = async () => {
    try {
      ...
      message.success(t('pg.linkCopied'));
    } catch {
      message.error(t('pg.copyFailed'));
    }
  };
    ...
    return (
      <div className='flex items-center justify-center h-screen bg-white dark:bg-[#111217]'>
        <div className='text-center space-y-3'>
          ...
          <p className='text-gray-400'>{t('pg.loading')}</p>
        </div>
      </div>
    );
  }
  ...
  if (fetchError || rounds.length === 0) {
    return (
      <div className='flex items-center justify-center h-screen bg-white dark:bg-[#111217]'>
        <div className='text-center space-y-3'>
          <p className='text-2xl font-semibold text-gray-700 dark:text-gray-200'>{t('pg.shareLinkInvalid')}</p>
          <p className='text-gray-400'>{fetchError || t('pg.noReplayContent')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <Head>
        <title>
          {firstQuestion
            ? t('pg.replayPageTitle', { question: `${firstQuestion.slice(0, 60)}${firstQuestion.length > 60 ? '…' : ''}` })
            : t('pg.replayPageTitleDefault')}
        </title>
      </Head>
          ...
          <div className='flex items-center gap-2'>
            ...
            {state.done ? (
              ...
              <Button type='primary' icon={<PlayCircleOutlined />} onClick={play}>
                {completedSteps === 0 ? t('pg.startReplay') : t('pg.continue')}
              </Button>
            )}

            {/* Skip to end */}
            {!state.done && (
              <Tooltip title={t('pg.skipToLastStep')}>
            ...
            <Tooltip title={t('pg.copyShareLinkTooltip')}>
              <Button
                icon={<LinkOutlined />}
        ...
        <div className='px-5 py-2 border-b border-gray-100 dark:border-gray-800/60 bg-gray-50/50 dark:bg-transparent flex-shrink-0'>
          <div className='flex items-center gap-3'>
            ...
            <span className='text-xs text-gray-400 whitespace-nowrap tabular-nums'>
              {t('pg.stepProgress', { completed: completedSteps, total: totalSteps })}
            </span>
            {/* Round selector tabs */}
            {rounds.length > 1 && (
              <div className='flex items-center gap-1'>
                {rounds.map((round, ri) => (
                  <Tooltip key={ri} title={round.humanText ? round.humanText.slice(0, 40) : t('pg.roundN', { n: ri + 1 })}>
                    <button
          ...
          <div className='w-[42%] min-w-[360px] border-r border-gray-200 dark:border-gray-800 flex flex-col overflow-y-auto'>
            ...
            {rounds.length > 1 && state.roundIndex < rounds.length - 1 && (
              <div className='px-4 py-3 text-xs text-gray-400 text-center animate-pulse'>
                {t('pg.roundsPending', { count: rounds.length - state.roundIndex - 1 })}
              </div>
            )}
