...
export default function AppContent() {
  ...
  const { run: operate } = useRequest(
    ...
    },
    {
      ...
      onSuccess: data => {
        if (data[2]?.success) {
          message.success(t('cst.operateSuccess'));
        }
        getListFiltered();
      },
    },
  ...
  const { run: updateAdmins, loading: adminLoading } = useRequest(
    async (params: { app_code: string; admins: string[] }) => await apiInterceptors(updateAppAdmins(params)),
    {
      manual: true,
      onSuccess: () => {
        message.success(t('cst.updateSuccess'));
      },
    },
  );
  ...
  return (
    <ConstructLayout>
      ...
      <Modal title={t('cst.permissionManagement')} open={adminOpen} onCancel={() => setAdminOpen(false)} footer={null}>
        <Spin spinning={loading}>
          <div className='py-4'>
            <div className='mb-1'>{t('cst.adminLabel')}</div>
            <Select
              mode='tags'
              value={admins}
              style={{ width: '100%' }}
