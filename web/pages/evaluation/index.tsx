...
const Evaluation = () => {
  ...
  const columns: TableProps<DataSetItemType>['columns'] = [
    ...
    {
      title: t('eval.storageMethod'),
      dataIndex: 'storage_type',
      key: 'storage_type',
    },
    {
      title: t('eval.datasetCount'),
      dataIndex: 'datasets_count',
      key: 'datasets_count',
    },

    {
      title: t('eval.createTime'),
      dataIndex: 'gmt_create',
      key: 'gmt_create',
    },
    {
      title: t('eval.members'),
      dataIndex: 'members',
      key: 'members',
      width: '10%',
      render: text => {
        ...
        <Space size='middle'>
          <Popconfirm
            ...
            onConfirm={async () => {
              ...
              if (res?.success == true) {
                message.success(t('eval.deleteSuccess'));
                getDataSetsRefresh();
              }
            }}
          >
            <Button type='link'>{t('eval.delete')}</Button>
          </Popconfirm>
          <Button
            type='link'
            onClick={() => {
  ...
  const evaluationsColumns: TableProps<EvaluationItemType>['columns'] = [
    ...
    {
      title: t('eval.evaluationMetrics'),
    ...
    Table.EXPAND_COLUMN,
    {
      title: (
        <span className='w-[50px]'>
          <span className='text-nowrap'>{t('eval.details')}</span>
          <Tooltip placement='topLeft' title={t('eval.viewLogsAndScores')}>
            <InfoCircleOutlined />
          </Tooltip>
        </span>
      ),
    ...
    >
      <div className='flex flex-col h-full w-full  dark:bg-gradient-dark bg-gradient-light bg-cover bg-center'>
        <div className='px-6 py-2 overflow-y-auto'>
          ...
          <Modal
            title={t('eval.startEvaluationModalTitle')}
            open={isModalOpen}
            onOk={async () => {
              ...
              if (values) {
                ...
                if (res?.success) {
                  message.success(t('eval.startSuccess'));
                  getEvaluationsRefresh();
                  form.resetFields();
                }
              }
          ...
          >
            <Form
              ...
              wrapperCol={{ span: 20 }}
            >
              <Form.Item name='scene_key' label={t('eval.sceneType')} rules={[{ required: true }]}>
                <Select
                  options={[
                    {
                      value: 'recall',
              ...
              <Form.Item name='scene_value' label={t('eval.sceneParameter')} rules={[{ required: true }]}>
                <Select
              ...
              <Form.Item name='parallel_num' label={t('eval.parallelParameter')} rules={[{ required: true }]} initialValue={1}>
                <Input></Input>
              </Form.Item>
              <Form.Item name='datasets' label={t('eval.dataset')} rules={[{ required: true }]}>
                <Select options={dataSetsOptions}></Select>
              </Form.Item>
              <Form.Item
                name='evaluate_metrics'
                label={t('eval.evaluateMetrics')}
          ...
          <Modal
            title={isAddDataSet ? t('eval.addDataset') : t('eval.editDataset')}
                ...
                if (isAddDataSet) {
                  ...
                  if (storageType === 'oss') {
                    ...
                    uploadDataSetsFile(formData)
                      .then(response => {
                        if (response.data.success) {
                          message.success(t('eval.uploadSuccess'));
                          runGetDataSets();
                        } else {
                      ...
                      .catch(error => {
                        ...
                        message.error(error?.response?.data?.err_msg || t('eval.uploadFailed'));
                      })
                      .finally(() => {
                    ...
                    uploadDataSetsContent({
                      ...
                      .catch(err => {
                        console.log(err);
                        message.error(err?.response?.data?.err_msg || t('eval.uploadFailed'));
                      })
                      .finally(() => {
                        setIsDataSetModalOpen(false);
                        setDataSetModalLoading(false);
                  ...
                  updateEvaluations({
                    ...
                    .then(res => {
                      if (res.data.success) {
                        message.success(t('eval.updateSuccess'));
                        runGetDataSets();
                      } else {
                        message.error(res.data.err_msg);
                      }
                    ...
                    .catch(err => {
                      console.log(err);
                      message.error(t('eval.updateFailed'));
                    })
                    .finally(() => {
                      setIsDataSetModalOpen(false);
                      setDataSetModalLoading(false);
          ...
          >
            <Form
              ...
              wrapperCol={{ span: 20 }}
            >
              <Form.Item name='dataset_name' label={t('eval.name')} rules={[{ required: true }]}>
                <Input disabled={!isAddDataSet} />
              </Form.Item>
              <Form.Item name='members' label={t('eval.members')}>
                <Select mode='tags' />
              </Form.Item>
              {isAddDataSet && (
                <Form.Item name='storage_type' label={t('eval.storageType')} rules={[{ required: true }]}>
