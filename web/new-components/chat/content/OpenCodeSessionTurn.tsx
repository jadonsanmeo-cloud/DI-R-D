...
const getFileTypeLabel = (fileName: string, mimeType?: string): string => {
  ...
  if (['xlsx', 'xls'].includes(ext) || mimeType?.includes('spreadsheet') || mimeType?.includes('excel')) {
    return i18n.t('chatc.fileSpreadsheet');
  }
  if (ext === 'csv' || mimeType?.includes('csv')) {
    return i18n.t('chatc.fileSpreadsheet');
  }
  if (ext === 'pdf' || mimeType?.includes('pdf')) return 'PDF';
  if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg'].includes(ext) || mimeType?.includes('image')) return i18n.t('chatc.artifactImage');
  if (['doc', 'docx'].includes(ext) || mimeType?.includes('word')) return i18n.t('chatc.fileWord');
  if (['txt', 'md'].includes(ext) || mimeType?.includes('text')) return i18n.t('chatc.fileTextFile');
  if (['json'].includes(ext)) return 'JSON';
  return i18n.t('chatc.artifactFile');
};
