...
const getArtifactTypeLabel = (type: string): string => {
  const map: Record<string, string> = {
    file: 'chatc.artifactFile',
    html: 'chatc.artifactHtml',
    table: 'chatc.artifactTable',
    chart: 'chatc.artifactChart',
    image: 'chatc.artifactImage',
    code: 'chatc.artifactCode',
    markdown: 'chatc.artifactMarkdown',
    summary: 'chatc.artifactSummary',
  };
  return i18n.t(map[type] || 'chatc.artifactGeneric');
};

type FileFilterTab = 'all' | 'document' | 'image' | 'code' | 'link';

const FILE_FILTER_TABS: { key: FileFilterTab; labelKey: string }[] = [
  { key: 'all', labelKey: 'chatc.filterAll' },
  { key: 'document', labelKey: 'chatc.filterDocument' },
  { key: 'image', labelKey: 'chatc.filterImage' },
  { key: 'code', labelKey: 'chatc.filterCode' },
  { key: 'link', labelKey: 'chatc.filterLink' },
];
...
const formatArtifactDate = (timestamp: number): string => {
  ...
  if (diffDays === 0) return i18n.t('chatc.today');
  if (diffDays === 1) return i18n.t('chatc.yesterday');
  if (diffDays < 7) {
    const dayNames = [
      'chatc.weekdaySun',
      'chatc.weekdayMon',
    ...
    return i18n.t(dayNames[date.getDay()]);
  }
  return i18n.t('chatc.monthDay', { month: date.getMonth() + 1, day: date.getDate() });
};
        ...
        <div className='flex-1 min-h-0 overflow-auto'>
          {scriptSource ? (
            ...
            <div className='flex flex-col items-center justify-center py-12 text-gray-500'>
              ...
              <span className='text-xs'>{i18n.t('chatc.loadingScript')}</span>
            </div>
          )}
        </div>
      </div>
    ...
    <div className='flex flex-col flex-1 min-h-0 overflow-hidden rounded-xl border border-gray-700/60'>
      ...
      <div className='flex items-center justify-between px-4 py-2.5 bg-[#1e2030] border-b border-gray-700/50 shrink-0'>
        ...
        <div className='flex items-center gap-2'>
          ...
          {allText && (
            <Tooltip title={i18n.t('chatc.copyAll')}>
              <button
...
const SkillCardRenderer: React.FC<{
  ...
  const handleDownload = useCallback(async () => {
    try {
      ...
      message.success(t('chatc.downloadSuccess'));
    } catch {
      message.error(t('chatc.downloadFailed'));
    } finally {
        ...
        <div className='px-5 py-4'>
          <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center gap-2 flex-shrink-0 ml-3'>
              <Tooltip title={t('chatc.downloadAsZip')}>
                <button
        ...
        >
          <div className='flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400'>
            ...
            {detailData?.tree?.children && (
              <span className='text-gray-400'>{t('chatc.itemsCount', { count: detailData.tree.children.length })}</span>
            )}
          </div>
        ...
        <div className='flex-1 min-w-0 overflow-auto p-4'>
          {fileContent ? (
            ...
            <div className='flex flex-col items-center justify-center py-12 text-gray-400'>
              ...
              <span className='text-xs'>{t('chatc.selectFileToView')}</span>
            </div>
          )}
        </div>
      </div>
...
const ManusRightPanel: React.FC<ManusRightPanelProps> = ({
  ...
  const handleExportPdf = () => {
    ...
    if (previewArtifact) {
      ...
      if (win) {
        ...
        message.error(t('chatc.popupBlocked'));
      }
    }
  };
  ...
  // and code+html pairs into tabbed views ({i18n.t('chatc.renderResult')} / {i18n.t('chatc.sourceCode')})
      ...
      >
        {panelView === 'skill-preview' && skillName ? (
          ...
          <div className='space-y-0'>
            ...
            {dateGroupedArtifacts.length > 0 ? (
              ...
              <div className='flex flex-col items-center justify-center py-16 text-gray-400'>
                ...
                <span className='text-sm'>{t('chatc.noFiles')}</span>
              </div>
            )}
          </div>
                                ...
                                <Tooltip title={t('chatc.copySql')}>
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      message.success(t('chatc.sqlCopied'));
                                    }}
                                  >
                                    <CopyOutlined className='text-xs' />
                                    <span>Copy</span>
              ...
              <>
                ...
                {isRunning ? (
                  <div className='flex flex-col items-center justify-center py-12 text-gray-400'>
                    ...
                    <span className='text-sm'>{t('chatc.executing')}</span>
                    <span className='text-xs text-gray-500 mt-1'>{t('chatc.executingHint')}</span>
                  </div>
                ) : (
                  <div className='flex flex-col items-center justify-center py-12 text-gray-400'>
                    <FileTextOutlined className='text-3xl mb-4' />
                    <span className='text-sm'>{t('chatc.noOutput')}</span>
                  </div>
                )}
              </>
            )}
          ...
          <div className='flex flex-col items-center justify-center h-full py-20 text-gray-400'>
            ...
            <span className='text-sm font-medium mb-1'>{t('chatc.selectStep')}</span>
