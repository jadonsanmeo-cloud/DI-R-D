...
7. Put only source-file arguments in `execution_arguments`; the runtime binds
   upstream step outputs deterministically.
8. Return only JSON-serializable values. Never return raw bytes, open file handles,
   ...
   another object.
9. Treat source and upstream files as read-only. Do not write files, launch
   subprocesses, mutate external state, or rely on nondeterministic side effects.
10. `tool_name` must be a valid Python identifier, and `source_code` must define
    one top-level function with exactly that name.
11. Import only standard-library modules or packages declared by
